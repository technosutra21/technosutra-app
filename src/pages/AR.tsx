...
import { AlertCircle, ArrowLeft, BookOpen, ChevronDown, ChevronUp, ExternalLink, Eye, MapPin, Pause, Play, RotateCcw, Sparkles } from 'lucide-react';
...
const AR = () => {
  ...
  // Check offline status and AR assets availability
  useEffect(() => {
    const checkOfflineStatus = async () => {
      setIsOffline(!navigator.onLine);

      try {
        // Check if AR dependencies are cached
        const arAssets = await offlineStorage.getAllARAssets();
        setArAssetsReady(arAssets.length >= 4);

        if (!navigator.onLine && arAssets.length < 4) {
  ...
  // Check if model exists (with offline support)
  useEffect(() => {
    const checkModel = async () => {
      try {
        // First check if model is cached offline
        ...
        // If not cached and online, try to fetch
        if (navigator.onLine) {
        ...
        >
          ...
          >
            <CyberButton
              ...
              disabled={!modelExists || isLoading || (isOffline && !arAssetsReady)}
            >
        ...
        >
          ...
          >
            <CyberButton
              ...
              size="icon"
              glowEffect
              className={`camera-control-glow w-16 h-16 rounded-full bg-black/95 border-2 backdrop-blur-xl transition-all duration-300 shadow-lg relative overflow-hidden ${isRotating
