...
self.addEventListener('fetch', event => {
  ...
  // Allow caching of MapTiler API resources
  const isMapTilerResource = url.hostname === 'api.maptiler.com';

  if (shouldSkipCache && !isMapTilerResource) {
    ...
    (async () => {
      try {
        ...
        if (networkResponse.ok && networkResponse.status === 200) {
          ...
          // Cache files under 5MB, or MapTiler resources under 10MB
          const sizeLimit = isMapTilerResource ? 10 * 1024 * 1024 : 5 * 1024 * 1024;
          if (size < sizeLimit && await canCache(size)) {
            try {
